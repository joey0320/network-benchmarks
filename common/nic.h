#define NIC_BASE 0x5000L
#define NIC_SEND_REQ (NIC_BASE + 0)
#define NIC_RECV_REQ (NIC_BASE + 8)
#define NIC_SEND_COMP (NIC_BASE + 16)
#define NIC_RECV_COMP (NIC_BASE + 24)
#define NIC_COUNTS (NIC_BASE + 32)
#define NIC_MACADDR (NIC_BASE + 40)
#define NIC_SEND_REQ_HACK (NIC_BASE + 80)
...
static inline void nic_post_send(uint64_t addr, uint64_t len)
{
	...
	reg_write64(NIC_SEND_REQ_HACK, request >> 32);
...
static inline uint64_t nic_complete_recv(void)
{
	return reg_read64(NIC_RECV_COMP);
}
